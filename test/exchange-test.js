...
var jwtBearerExchange = require('oauth2orize-jwt-bearer/exchange');
...
vows.describe('jwtBearerExchange').addBatch({

  'middleware': {
    topic: function() {
      return jwtBearerExchange(function() {});
    },
  ...
  'middleware that issues an access token': {
    topic: function() {
      return jwtBearerExchange(function(client, jwt, done) {
    ...
    'when handling a request': {
      topic: function(jwtBearerExchange) {
        var self = this;
        ...
        process.nextTick(function () {
          jwtBearerExchange(req, res, next);
        });
      },
  ...
  'middleware that issues an access token based on data and signature': {
    topic: function() {
      return jwtBearerExchange(function(client, data, signature, done) {
  ...
  'middleware that issues an access token based on header, claimSet and signature': {
    topic: function() {
      return jwtBearerExchange(function(client, header, claimSet, signature, done) {
  ...
  'middleware with userProperty option that issues an access token': {
    topic: function() {
      return jwtBearerExchange({ userProperty: 'otheruser' }, function(client, jwt, done) {
        if (client.id == 'c123' && jwt == 'header.claimSet.signature') {
          done(null, 's3cr1t');
        } else {
          return done(new Error('something is wrong'));
  ...
  'middleware constructed without an issue function': {
    'should throw an error': function () {
      assert.throws(function() { jwtBearerExchange(); });
    }
  }
